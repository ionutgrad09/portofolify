...
import {formatRON} from "../utils/utils";
    ...
    <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-800">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={cumulativeData}>
          ...
          <YAxis stroke="#94a3b8" tick={{fontSize: 11}} tickFormatter={(val: number) => formatRON(val)}/>
          <Tooltip
            contentStyle={{backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px', color: "white"}}
            formatter={(value: any) => [formatRON(value), 'Profit Cumulativ']}
          />
      ...
      <p className="text-slate-400 text-sm mt-4 text-center">
        ...
        className={`font-bold ${cumulativeData[cumulativeData.length - 1]?.cumulative >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          {formatRON(cumulativeData[cumulativeData.length - 1]?.cumulative || 0)}
        </span>
      </p>
    </div>
  );
